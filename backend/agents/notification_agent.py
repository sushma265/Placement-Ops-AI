...
class NotificationAgent:
    ...
    def send_eligibility_notification(cls, student_id: int, drive_id: int, eligible: bool, reason: str, db: Session) -> bool:
        ...
        db.add(notification)
        db.commit()
        logger.info(f"[SMTP Simulator] Sent email notification to {student.email} successfully.")
        return True
    ...
    def send_interview_notification(cls, interview_id: int, db: Session) -> bool:
        ...
        for panelist in interview.panel_members:
            ...
            db.add(Notification(
                ...
                # NOTE (known gap, not a bug): there is no Panelist/staff entity
            # in the schema -- Interview.panel_members is just free-text
            # names (e.g. "Dr. Prasad"), with no real ID to reference here.
            # recipient_id is a placeholder until a real Panelist model
            # exists; these panel notifications aren't retrievable by any
            # panelist-facing view today (only students see their own
            # notifications, via recipient_type == "student").
            recipient_id=999,
                channel="email",
        ...
        db.commit()
        logger.info(f"[SMTP Simulator] Sent notifications for interview ID {interview_id} to student and panel.")
        return True
