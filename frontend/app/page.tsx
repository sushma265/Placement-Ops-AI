...
import {
  ...
  ChevronRight, CircleHelp, ClipboardCheck, FileText,
  LayoutDashboard, Menu, Moon, MoreHorizontal, MoveRight, Plus, Search,
  Sparkles, Target, Users, X, Zap, BarChart3, Building2, Send, RotateCcw,
  FileSpreadsheet, Mail, ShieldAlert, LogOut, Landmark
} from 'lucide-react'
...
export default function Page() {
  ...
  return (
    <>
      ...
      {view === 'app' && user?.role === 'student' && <StudentDashboard user={user} onLogout={handleLogout} />}
      {view === 'app' && user?.role !== 'student' && <Dashboard user={user} onLogout={handleLogout} />}
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        onLoginSuccess({ role: synced.role, user: { ...profile, ...synced, id: authUser.id } });
      } else {
        ...
        const synced = await syncProfile(profile?.role || role);
        onLoginSuccess({ role: synced.role, user: { ...profile, ...synced, id: authUser.id } });
      }
    } catch (err: any) {
...
function Dashboard({ user, onLogout }: { user: any; onLogout: () => void }) {
  ...
  useEffect(() => {
    // Re-route views depending on the logged in role. Dashboard is only
    // ever rendered for recruiter/tpo now -- students are routed straight
...
function RecruiterShortlistScreen({ isOnline, activeDrive, drives, onSelectDrive }: { isOnline: boolean; activeDrive: any | null; drives: any[]; onSelectDrive: (id: number) => void }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/drives/${activeDrive.id}/shortlist`)
      .then(res => res.json())
...
// ----------------------------------------------------
function RecruiterScheduleScreen({ isOnline, activeDrive, drives: _drives, onSelectDrive: _onSelectDrive }: { isOnline: boolean; activeDrive: any | null; drives: any[]; onSelectDrive: (id: number) => void }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/drives/${activeDrive.id}/interviews`)
      .then(res => res.json())
...
// ----------------------------------------------------
function DashboardHome({ onGo, isOnline: _isOnline, drives, exceptionsCount, onSelectDrive }: { onGo: (s: string) => void; isOnline: boolean; drives: any[]; exceptionsCount: number; onSelectDrive: (id: number) => void }) {
  const actions = [
...
function JDIntakeScreen({ isOnline, drives, activeDriveId, onSelectDrive, onRefreshDrives }: { isOnline: boolean; drives: any[]; activeDriveId: number | null; onSelectDrive: (id: number) => void; onRefreshDrives: () => void }) {
  ...
  const handleStartIntake = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiFetch(`/drives`, {
        method: 'POST',
  ...
  const handleConfirmPublish = async () => {
    ...
    try {
      const response = await apiFetch(`/drives/${draftDrive.id}/confirm`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
function EligibilityScreen({ isOnline, activeDrive, drives, onSelectDrive }: { isOnline: boolean; activeDrive: any | null; drives: any[]; onSelectDrive: (id: number) => void }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/drives/${activeDrive.id}/eligibility`)
      .then(res => res.json())
  ...
  const handleOverrideSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiFetch(`/eligibility/${overrideModal.record.eligibility_id}/override`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
function MatchingScreen({ isOnline, activeDrive, drives, onSelectDrive }: { isOnline: boolean; activeDrive: any | null; drives: any[]; onSelectDrive: (id: number) => void }) {
  ...
  const handleApproveShortlist = async () => {
    ...
    try {
      const response = await apiFetch(`/drives/${activeDrive.id}/shortlist/approve`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
function SchedulingScreen({ isOnline, activeDrive, drives, onSelectDrive }: { isOnline: boolean; activeDrive: any | null; drives: any[]; onSelectDrive: (id: number) => void }) {
  ...
  const handlePropose = async () => {
    ...
    try {
      const response = await apiFetch(`/drives/${activeDrive.id}/schedule/propose`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleConfirm = async () => {
    ...
    try {
      const response = await apiFetch(`/schedule/${activeDrive.id}/confirm`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiFetch(`/interviews/${editModal.record.interview_id}/resolve`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
...
function ExceptionsScreen({ isOnline, onUpdateException }: { isOnline: boolean; onUpdateException: () => void }) {
  ...
  const handleResolve = async (id: number) => {
    ...
    try {
      await apiFetch(`/exceptions/${id}/resolve`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
function AnalyticsScreen({ isOnline }: { isOnline: boolean }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/analytics/skill-gap`)
      .then(res => res.json())
      .then(data => setSkillGap(data))
      .catch(err => console.error(err));

    apiFetch(`/analytics/readiness-trend`)
      .then(res => res.json())
...
function ReportsScreen({ isOnline, drives }: { isOnline: boolean; drives: any[] }) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/reports/${selectedReportDriveId}`)
      .then(res => res.json())
      .then(data => {
        setReport(data);
        setLoading(false);
  ...
  const handleDownloadCsv = async () => {
    if (!selectedReportDriveId) return;
    try {
      const res = await apiFetch(`/reports/${selectedReportDriveId}/csv`);
      if (!res.ok) throw new Error('Failed to download report.');
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `drive_${selectedReportDriveId}_placement_report.csv`;
  ...
  return (
    <div className="motion-page">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        ...
        <div className="flex gap-2">
          ...
          {selectedReportDriveId && (
            <button onClick={handleDownloadCsv} className="btn btn-primary text-xs flex items-center gap-1 font-semibold">
      ...
      {loading && (
        <div className="text-xs text-muted-foreground mb-4">Loading report…</div>
      )}
